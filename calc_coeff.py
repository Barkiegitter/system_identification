...
df_main['beta'] = np.rad2deg(np.arctan((df_main.u)/(0.7*np.pi*df_main.rpm*ship.D_p)))
 ...
 # F_p(i) = (1-t)*rho * n_p(i)^2 * D_p^4 * interp1(K_t(:,1),K_t(:,2),J(i),'linear','extrap');
...
# df_main.beta = df_main.beta.rolling(19).mean()

# df_main = df_main[20:]

# df_main['f_p_40'] = (1-ship.t)*ship.beta_coef(df_main.beta)*ship.rho*ship.D_p**4*df_main.rpm**2
...
y_x = ship.Mass*(u_dot-r*v)-2.*f_p_40 - F_r*np.sin(np.deg2rad(rsa))
...
# lasso = Ridge(fit_intercept=False )
...
print(results_x.best_estimator_.score(X,y_x))
print(results_y.best_estimator_.score(Y,y_y))
print(results_r.best_estimator_.score(N,y_r))



plt.plot(np.sum(X*results_x.best_estimator_.coef_, axis=1));plt.plot(y_x)
plt.show()
plt.close()
