...
mean_period_delta_psi = 20 #20
shift_period_delta_psi = int(mean_period/2)

mean_period_dot = 50 #10
shift_period_dot = int(mean_period/2)


# differente filtering voor u en v/r. Daarbij check de gewichtenv an RT evolution, lijkt wel meer

df_main = pd.read_csv('RTBorkum_turbopolyp.csv', sep=',')#[2000:]
...
# df_main = df_main[abs(df_main.x)<1.0]
# df_main = df_main.reset_index(drop=True)
...
P = np.identity(3)*0
#measurement noise
Q = np.diag([5.,.5,0.0005])
H = np.identity(3)
R = np.diag([100., 100., .01])
B = 0
u = 0
...
df_main.x = df_main.x.shift(-2)
df_main.y = df_main.y.shift(-2)
df_main.delta_psi = df_main.delta_psi.shift(-3) #wellicht hier perfecitoneren
plt.plot(df_main.x.tolist())
...
df_main.x = df_main.x.shift(-int(mean_period/1))
df_main.y = df_main.y.shift(-int(mean_period/1))
df_main.delta_psi = df_main.delta_psi.shift(-int(mean_period_delta_psi/2))

plt.plot(df_main.x.tolist())
# plt.ylabel('delta distance travelled in x direction')
# plt.xlabel('time')
# plt.title('Filter')
# plt.savefig('filterexample.png')
plt.show()
...
df_main=df_main.dropna(inplace=False)
states = df_main[['u', 'v', 'r']].to_numpy()
states = states.reshape([states.shape[0], 3, 1])
# add z score filtering
A = np.identity(3)
P = np.identity(3)*0
#measurement noise
Q = np.diag([0.01,.5,0.03])
H = np.identity(3)
R = np.diag([.5, 100., 1.0])
B = 0
u = 0
...
plt.plot(df_main.r.tolist())
df_main.u = new_states[:, 0, :];
df_main.v = new_states[:, 1, :];
df_main.r = new_states[:, 2, :]
plt.plot(new_states[:,2,:])

df_main.u = df_main.u.shift(-12)
df_main.v = df_main.v.shift(-12) #this could be an issue
df_main.r = df_main.r.shift(-5) #recheck this
plt.plot(df_main.r.tolist())
...
df_main.r = df_main.r.shift(-int(mean_period))
...
states = df_main[['u_dot', 'v_dot', 'r_dot']].to_numpy()
states = states.reshape([states.shape[0], 3, 1])
# add z score filtering
A = np.identity(3)
P = np.identity(3)*0
#measurement noise
Q = np.diag([0.8,0.1,0.02])
H = np.identity(3)
R = np.diag([10., 1.0, 0.2])
B = 0
u = 0
x = inv(H).dot(states[0])
P = inv(H).dot(R).dot(inv(H.T))
...
df_main.u_dot = new_states[:, 0, :];
df_main.v_dot = new_states[:, 1, :];
df_main.r_dot = new_states[:, 2, :]
plt.plot(new_states[:,2,:])

df_main.u_dot = df_main.u_dot.shift(-1)
df_main.v_dot = df_main.v_dot.shift(-1)
df_main.r_dot = df_main.r_dot.shift(-2)
plt.plot(df_main.r_dot.tolist())
...
df_main.u_dot = df_main.u_dot.shift(-int(mean_period_dot))
df_main.v_dot = df_main.v_dot.shift(-int(mean_period_dot))
df_main.r_dot = df_main.r_dot.shift(-int(mean_period_dot))
plt.plot(df_main.r_dot.tolist())
...
df_main = df_main[abs(df_main.rsa_0_diff)<5.]
df_main = df_main[abs(df_main.rsa_1_diff)<5.]

df_main = df_main[abs(df_main.rsa_2_diff)<5.]
...
# df_main.to_csv('test_1_large_turbopolyp.csv', index=False)


# r_dot all over the place
