...
ship_elements = [(0.5,1.2), (0.5, 1.5), (0.1, 10.0), (0.5,1.5), (0.5,1.5), (0.5,1.5)]
...
def traj_error(x):
    ...
    ship_it.I_e = ship_it.I_e*x[2]
    ship_it.x_1, ship_it.x_2 = ship_it.x_1*x[4], ship_it.x_2*x[4]
    ship_it.y_1, ship_it.y_2 = ship_it.y_1*x[5], ship_it.y_2*x[5]
    ship_it.x_0 = ship_it.x_0*x[5]
    # ship_it.D_p = ship_it.D_p*x[6]
    ...
    df['u_a_2'] = (1-ship_it.w)*((df.u+df.r*abs(ship_it.y_2))*-1*np.cos(np.deg2rad(df.rsa_2)) + (df.v+df.r*abs(ship_it.x_2))*-1*np.sin(np.deg2rad(df.rsa_2))) #(1-ship_it.w)*
    df['u_a_1'] = (1-ship_it.w)*((df.u-df.r*abs(ship_it.y_1))*-1*np.cos(np.deg2rad(df.rsa_1)) + (-df.v-df.r*abs(ship_it.x_1))*-1*np.sin(np.deg2rad(df.rsa_1))) #(1-ship_it.w)*
    df['u_a_0'] = (1-ship_it.w)*((df.u)*-1*np.cos(np.deg2rad(df.rsa_0)) + ((-df.v + df.r*abs(ship_it.x_0))*-1*np.sin(np.deg2rad(df.rsa_0))) ) #(1-ship_it.w)*
    ...
    df['f_p_40_2'] = x[0]*1.*((1-ship_it.t)*ship_it.beta_coef(df.beta_2)*0.5*ship_it.rho*(((((1-ship_it.w)*df.u_a_2)**2)+(0.7*np.pi*df.rpm_2*ship_it.D_p)**2))*np.pi/4*ship_it.D_p**2)  #(1-df['t_21_phi'])*(1-df['t_20_phi'])*
    df['f_p_40_1'] = x[0]*1.*((1-ship_it.t)*ship_it.beta_coef(df.beta_1)*0.5*ship_it.rho*(((((1-ship_it.w)*df.u_a_1)**2)+(0.7*np.pi*df.rpm_1*ship_it.D_p)**2))*np.pi/4*ship_it.D_p**2) #(1-df['t_12_phi'])*(1-df['t_10_phi'])*
    df['f_p_40_0'] = x[0]*1.*((1-ship_it.t)*ship_it.beta_coef(df.beta_0)*0.5*ship_it.rho*(((((1-ship_it.w)*df.u_a_0)**2)+(0.7*np.pi*df.rpm_0*ship_it.D_p)**2))*np.pi/4*ship_it.D_p**2)#.rolling(20).mean()  #(1-df['t_02_phi'])*(1-df['t_01_phi'])*
    ...
    y_r = ship_it.I_z*r_dot-1*(ship_it.x_0*-1*np.sin(np.deg2rad(rsa_0))*(f_p_40_0)+ship_it.x_2*-1*np.sin(np.deg2rad(rsa_2))*(f_p_40_2) + ship_it.x_1*-1*np.sin(np.deg2rad(rsa_1))*(f_p_40_1) - ship_it.y_2*-1*np.cos(np.deg2rad(rsa_2))*(f_p_40_2) - ship_it.y_1*-1*np.cos(np.deg2rad(rsa_1))*(f_p_40_1))
    ...
    # feature = results_r.best_estimator_.score(N,y_r)*results_y.best_estimator_.score(Y,y_y)*results_x.best_estimator_.score(X,y_x)
    coef__ = np.array([np.asarray(results_x.best_estimator_.coef_[0]),np.asarray(results_y.best_estimator_.coef_[0]),np.asarray(results_r.best_estimator_.coef_[0])])
    print(coef__)
