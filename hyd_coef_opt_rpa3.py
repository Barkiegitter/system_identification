...
df_main = df_main.dropna()[:750]
coef__ = np.genfromtxt('foo_rpa3.csv', delimiter=',')
...
for coefficient in first_round_coef:
    # coef_bounds = (min(0,coefficient),max(0,coefficient) )#abs(coef/1000.0))
    coef_bounds = (coefficient -abs(coefficient/100.0) ,coefficient + abs(coefficient/100.0))#abs(coef/1000.0))
    bounds.append(coef_bounds)
    

traj_seg_len = [50,100, 250, 500, 650, 10000]#5, 10, 25, 100, 
nv = len(bounds)   
...
for traj_len in traj_seg_len:
    def traj_error(x):
        ...
        coef__ = np.array([np.asarray(x[:11]),np.asarray(x[11:25]),np.asarray(x[25:])])
        # print(coef__)
    ...
    for i_coef in range(len(it_coef)):
        ...
        coef_bounds = (it_coef[i_coef] - coef_delta*1.05,it_coef[i_coef] + coef_delta*1.05 )#abs(coef/1000.0))
        bounds.append(coef_bounds)
